...
- **Segurança**: `com.confeitaria.config.SecurityConfig`
  - Usuário em memória:
    - `admin` / `confeitaria123` (role `ADMIN`)
  - Regras:
...
- **Credenciais fixas no código**: usuário e senha do admin estão hardcoded em `SecurityConfig`.
  - Risco: qualquer pessoa com acesso ao repositório conhece a senha.
  - Melhor prática: mover para variáveis de ambiente / arquivo de config + usuário no banco.
